...
class AppearanceOptimizer:
    def __init__(self, gaussians: GaussianState, min_scale: float = 0.005, max_scale: float = 0.01):
        ...
        self.optimizer = Adam(
            [
                ...
                wp.from_torch(self.gaussians.opacities_logits, dtype=wp.float32).flatten(),
    ...
    def step(self):
        self.optimizer.step(
            grad=[
                wp.from_torch(self.gaussians.colors_logits.grad, dtype=wp.float32).flatten(),
                wp.from_torch(self.gaussians.opacities_logits.grad, dtype=wp.float32).flatten(),
